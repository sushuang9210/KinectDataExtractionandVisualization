...
#include <stdlib.h>
using namespace std;
std::ifstream ifs("E:\\input.txt");
std::ofstream ofs("E:\\cup.txt");
int* index;
int* label;
int n_object;
//ofstream ofs;
    ...
    m_pColorRGBX(NULL)
{
	...
	camerapoint =new CameraSpacePoint[cDepthWidth * cDepthHeight];
}
...
void CCoordinateMappingBasics::Update()
{
	...
	float* depth_x;
	float* depth_y;
	depth_x = (float*)malloc(sizeof(float)*n_object);
	depth_y = (float*)malloc(sizeof(float)*n_object);
	label = (int*)malloc(sizeof(int)*n_object);
	index = (int*)malloc(sizeof(int)*n_object);
	for (int i = 0; i < n_object; i++)
	{
       ifs >> label[i]>>depth_x[i] >> depth_y[i];
	   index[i] = depth_y[i] * 512 + depth_x[i];
	}
	ifs.seekg(0);
                                            ...
                                            const BYTE* pBodyIndexBuffer, int nBodyIndexWidth, int nBodyIndexHeight)
{
        ...
        pBodyIndexBuffer && (nBodyIndexWidth == cDepthWidth) && (nBodyIndexHeight == cDepthHeight))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
				if (index[j] == depthIndex)
				{
					m_pCoordinateMapper->MapDepthFrameToCameraSpace(nDepthWidth * nDepthHeight,(UINT16*) pDepthBuffer, nDepthWidth * nDepthHeight, camerapoint);
					CameraSpacePoint a = camerapoint[depthIndex];
					ofs << label[j] <<' '<< a.X << ' '<<a.Y <<' '<<a.Z << '\n';
					j = j + 1;
				}
