...
void CFaceBasics::Update()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {   
			...
			GetScreenshotFileName(szScreenshotPath, _countof(szScreenshotPath), timestamp, 0);
			//i = i + 1;
		...
		if (SUCCEEDED(hrD))
		{
			...
			if (SUCCEEDED(hrD))
			{
				...
				if (SUCCEEDED(hrD))
				{
					hrD = pDepthFrameDescription->get_Width(&nDepthWidth);
				}

				if (SUCCEEDED(hrD))
				{
					hrD = pDepthFrameDescription->get_Height(&nDepthHeight);
				}
				...
				if (SUCCEEDED(hrD))
				{
					...
					pDepthBuffer = m_pDepthData;
					nDepthBufferSize = nDepthWidth * nDepthHeight * sizeof(UINT16);
					// Retrieve the path to My Photos
					GetScreenshotFileName(szScreenshotPath, _countof(szScreenshotPath), timestamp, 1);
					//i = i + 1;
					// Write out the bitmap to disk
					// comment out to discard of frame dump to file
					hrD = SaveBitmapToFile(reinterpret_cast<BYTE*>(pDepthBuffer), nDepthWidth, nDepthHeight, sizeof(UINT16) * 8, szScreenshotPath);
				}
			}
...
HRESULT CFaceBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize, int i, int depth_fl)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (depth_fl == 1)
		{
			StringCchPrintfW(lpszFilePath, nFilePathSize, L"%s\\depth_%d.bmp", pszKnownPath, i);
		}
		else
		{
