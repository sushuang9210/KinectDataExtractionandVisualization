...
time_t now = time(0);
INT64 nTime = 0;
std::ofstream ofs("C:\\Kinect Data\\time.txt");
//std::ofstream ofs1("C:\\Kinect Data\\skeleton.txt");
/// <summary>
...
int CColorBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	fs = FileStorage(".\\apple.xml", FileStorage::WRITE);
    // Main message loop
    while (WM_QUIT != msg.message)
    {
		tm *timelocate = localtime(&now);
		hour = timelocate->tm_hour;
		minute = timelocate->tm_min;
		second = timelocate->tm_sec;
		timetotal = 3600 * hour + 60 * minute + second;
        Update();
    ...
    if (SUCCEEDED(hr))
    {
       // INT64 nTime = 0;
		...
		if (SUCCEEDED(hr))
		{
			...
			for (int p = 0; (p < 6) && (!bTracked); ++p)
			{
				IBody* pBody = ppBodies[p];
				if (pBody)
				{
					//ofs << 2 << '\n';
					//BOOLEAN bTracked = false;
        ...
        if (bTracked)
		{
			...
			if (state)
			{
				timetotal_pre = timetotal;
				state = 0;
			}
			else if ((timetotal - timetotal_pre) == 12)
			{
				timetotal_pre = timetotal;
		...
		colorBuffer = new RGBQUAD[nHeight * nWidth];
		hr = pColorFrame->CopyConvertedFrameDataToArray(nHeight * nWidth * sizeof(RGBQUAD), reinterpret_cast<BYTE*>(colorBuffer), ColorImageFormat_Bgra);
		if (SUCCEEDED(hr))
		{
			cv::Mat img1(nHeight, nWidth, CV_8UC4, reinterpret_cast<void*>(colorBuffer));
...
void CColorBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight) 
{
    ...
    if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
    {
      ...
      //  if (m_bSaveScreenshot)
			...
			GetScreenshotFileName(szScreenshotPath, _countof(szScreenshotPath), i);
			i = i + 1;
...
/// </returns>
HRESULT CColorBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize,int i)
{
