...
std::ofstream ofs_temp_out;
char infile[MAX_PATH], outfile[MAX_PATH], tempoutfile[MAX_PATH];
bool convert_once_fl = FALSE;
bool export_once_fl = FALSE;
    ...
    m_pBodyFrameReader(nullptr)	
{
	...
	m_color2depthLUT = new DepthSpacePoint[cColorWidth * cColorHeight];
	for (int i = 0; i < (cColorWidth * cColorHeight); i++)
	{
		m_color2depthLUT[i].X = -1;
		m_color2depthLUT[i].Y = -1;
	}
	...
	sprintf_s(tempoutfile, "C:\\Kinect Data\\temp_out.csv");
	ofs_temp_out.open(tempoutfile);
	//////////////////////////////////////////////////
}
...
//					ofs << "sk: Joints (X,Y,Z,state); Color point (X,Y); Depth point (X,Y,D); " << iFace << ":\n";
					...
					for (int j = 0; j < _countof(joints); ++j)
					{
						...
						ofs << "joint " << j << ":" << '\n';
						ofs << "Camera Space:" << "(" << joints[j].Position.X << " " << joints[j].Position.Y << " " << joints[j].Position.Z << ' ' << joints[j].TrackingState << ")"<<'\n';
						ofs << "Color Space:";
...
//            }
			if (bFaceTracked)
			{
				...
				if (SUCCEEDED(hr) && pFaceFrameResult != nullptr)
				{
					...
					for (int j = 0; j < _countof(facePoints); ++j)
					{
						...
						ofs << "Camera Space:" << "(" << cameraPoint.X << " " << cameraPoint.Y << " " << cameraPoint.Z << ")" << '\n';
						ofs << "Depth Space:" << "(" << depthPoint.X << " " << depthPoint.Y << " " << depthVal << ")" << '\n';
						ofs << "Color Space:" <<"(" << facePoints[j].X << " " << facePoints[j].Y << ")"<<'\n';
					}
					ofs << '\n';
					//						ofs << "(" << facePoints[0].X << "," << facePoints[0].Y << "),(" << facePoints[1].X << "," << facePoints[1].Y << "),(" << facePoints[2].X << "," << facePoints[2].Y << "),(" << facePoints[3].X << "," << facePoints[3].Y << "),(" << facePoints[4].X << "," << facePoints[4].Y << ")," << '\n';
					ofs << "face bbox: bottom (X,Y); left (X,Y); right (X,Y); top (X,Y);" << '\n';
		...
		if (export_once_fl)
		{
			...
			hr = m_pCoordinateMapper->MapColorFrameToDepthSpace(cDepthWidth * cDepthHeight, m_pDepthData, cColorWidth * cColorHeight, depthSpacePoints);
			int valid_cntr = 0;
			for (int j = 0; j < (cColorWidth * cColorHeight); ++j)
			{
				if (depthSpacePoints[j].X >= 0)
				{
					m_color2depthLUT[j] = depthSpacePoints[j];
				}
				if (m_color2depthLUT[j].X >= 0)
				{
					valid_cntr++;
				}
			}
			...
			if (valid_cntr > 0.69*(cColorWidth * cColorHeight))
			{
				ofs_temp_out << cColorWidth << "," << cColorHeight << '\n';
				for (int j = 0; j < (cColorWidth * cColorHeight); ++j)
				{
					ofs_temp_out << (((_fpclass(m_color2depthLUT[j].X) == _FPCLASS_NINF) || (_fpclass(m_color2depthLUT[j].X) == _FPCLASS_PINF)) ? -1 : m_color2depthLUT[j].X) << ",";
					ofs_temp_out << (((_fpclass(m_color2depthLUT[j].Y) == _FPCLASS_NINF) || (_fpclass(m_color2depthLUT[j].Y) == _FPCLASS_PINF)) ? -1 : m_color2depthLUT[j].Y) << '\n';
				}

				ofs_temp_out.close();
				export_once_fl = FALSE;
			}
		}
